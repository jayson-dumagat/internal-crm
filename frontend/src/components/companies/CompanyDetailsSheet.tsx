...
export default function CompanyDetailsSheet({
  ...
  return (
    <Sheet
      ...
      description="Review company relationship information."
      side="right"
    ...
    >
      {company && (
        <div className="space-y-6">
          ...
          <section className="rounded-xl border border-gray-100 p-4 dark:border-white/[0.05]">
            ...
            <div className="grid gap-x-5 gap-y-4 sm:grid-cols-2">
              {[
                ...
                ["Customer since", formatDisplayDate(company.customerSince)],
                ["Last activity", formatDisplayDate(company.lastActivity)],
            ...
            {company.contacts.length ? (
              <div className="flex flex-wrap gap-3">
                {company.contacts.map((contact) => (
                  <div
                    key={contact.name}
                    className="flex items-center gap-2 rounded-lg bg-gray-50 px-2.5 py-2 dark:bg-white/[0.03]"
                  >
                    <Avatar
                      src={contact.avatar}
                      alt={contact.name}
                      size="xsmall"
                    />
                    <span className="max-w-40 truncate text-sm text-gray-700 dark:text-gray-300">
                      {contact.name}
                    </span>
                  </div>
                ))}
          ...
          <div className="flex justify-end border-t border-gray-100 pt-5 dark:border-white/[0.05]">
            <button
              type="button"
              disabled={!canUpdate}
              onClick={() => onEdit(company as unknown as CompanyRecord)}
              className="inline-flex h-10 items-center rounded-lg bg-brand-500 px-4 text-sm font-medium text-white shadow-theme-xs transition hover:bg-brand-600 disabled:cursor-not-allowed disabled:opacity-40"
            >
              Edit company
            </button>
          </div>
