...
export default function NotesToolbar({ category, canCreate, onCategoryChange, onAdd }: NotesToolbarProps) {
  return (
      ...
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        ...
        <div className="flex shrink-0 items-center justify-end gap-2 [&_svg]:size-4">
          <div className="relative min-w-0 flex-1 sm:flex-none">
            ...
            <select value={category} onChange={(event) => onCategoryChange(event.target.value as NoteRecord["category"] | "All")} className="h-9 w-full appearance-none rounded-lg border border-gray-300 bg-white py-2 pr-9 pl-9 text-sm font-medium text-gray-700 shadow-theme-xs sm:w-44 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400">
              <option value="All">All categories</option>
              <option>Client</option><option>Follow-up</option><option>Investment</option><option>Internal</option>
            </select>
          </div>
          <button type="button" disabled={!canCreate} title={canCreate ? "Add Note" : "Read-only access"} onClick={onAdd} className="inline-flex h-9 shrink-0 items-center justify-center gap-2 rounded-lg bg-brand-500 px-3 text-sm font-medium text-white shadow-theme-xs hover:bg-brand-600 disabled:cursor-not-allowed disabled:opacity-40">
            <PlusIcon className="size-4" /><span className="hidden sm:inline">Add Note</span>
          </button>
        </div>
      </div>
    </div>
