...
import { createComplianceInputSchema, type CreateComplianceInput } from "../validations/brokerage";
import BrokerageWorkflowNav from "../components/brokerage/BrokerageWorkflowNav";
...
const caseTypes = ["kyc_renewal", "suspicious_activity", "unusual_transaction", "watchlist", "suitability_exception", "dormant_account", "complaint", "supervisor_approval", "manual_override"] as const;
const statuses = ["open", "in_review", "escalated", "resolved", "closed"] as const satisfies readonly CreateComplianceInput["status"][];
...
export default function Compliance() {
  ...
  const setStatus = async (id: string, status: (typeof statuses)[number]) => { try { await update.mutateAsync({ id, input: { status } }); toast.success("Case status updated."); } catch (error) { toast.error(error instanceof Error ? error.message : "Unable to update case."); } };
  const handleStatusChange = (id: string, value: string) => {
    const nextStatus = statuses.find((candidate) => candidate === value);
    if (nextStatus) void setStatus(id, nextStatus);
  };
  return <>
