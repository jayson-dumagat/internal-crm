  ...
  try {
    ...
    for (const lead of leads)
      relatedAvatars.set(
        ...
        lead.avatarUrl ? `/api/v1/leads/${lead.id}/avatar` : null,
      );
    for (const contact of contacts)
      relatedAvatars.set(
        (contact.name ?? "").toLowerCase(),
        contact.avatarUrl ? `/api/v1/contacts/${contact.id}/avatar` : null,
      );
    for (const company of companies)
      relatedAvatars.set(
        company.name.toLowerCase(),
        company.logoUrl ? `/api/v1/companies/${company.id}/logo` : null,
      );
    const currentUser = await getAuthor(req);
    const visibleNotes = notes
      .filter((note) => canAccessRecord(req, "notes", note.id))
      .filter((note) =>
        getDataScope(req, "notes") === "own"
      ...
      .filter((note) =>
        matchesDateRange(note.createdAt, query.dateFrom, query.dateTo),
      )
      .filter((note) =>
        matchesSearch(
          [
      ...
      .json({
        data: page.data.map((note) =>
          toNoteDto(
            note,
            req,
            relatedAvatars.get((note.relatedTo ?? "").toLowerCase()),
          ),
        ),
        meta: page.meta,
      });
...
export async function createNote(
  ...
  try {
    if (hasResourceRestriction(req, "notes")) {
      res
        ...
        .json({
          success: false,
          message:
            "You cannot create notes while note access is restricted to assigned records.",
        });
      return;
    }
    const forbiddenField = firstHiddenInput(req, req.body, {
      ...
      contentHtml: "notes.content",
      category: "notes.category",
      relatedTo: "notes.relatedTo",
    });
    if (forbiddenField) {
      res
        .status(403)
    ...
    if (!parsed.success) {
      res
        ...
        .json({
          success: false,
          message: "Please check the note fields and try again.",
    ...
    const note = noteRepository().create({
      ...
      authorAvatarUrl: author?.avatarUrl
        ? `/api/v1/users/${author.entraObjectId}/avatar`
        : null,
    });
...
export async function updateNote(
  ...
  try {
    ...
    if (!canAccessRecord(req, "notes", note.id)) {
      res.status(404).json({ success: false, message: "Note not found." });
      return;
    }
    ...
    if ("relatedTo" in parsed.data)
      note.relatedTo = parsed.data.relatedTo || null;
    if ("contentHtml" in parsed.data)
      note.contentHtml = parsed.data.contentHtml || null;
    const saved = await noteRepository().save(note);
...
export async function deleteNote(
  ...
  try {
    ...
    if (!canAccessRecord(req, "notes", String(req.params.id))) {
      res.status(404).json({ success: false, message: "Note not found." });
      return;
    }
    const result = await noteRepository().delete({
      id: String(req.params.id),
      tenantId,
    });
    if (!result.affected) {
