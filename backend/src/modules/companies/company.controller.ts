...
export async function listCompanies(
  ...
  try {
    ...
    const visibleCompanies = companies
      .filter((company) => canAccessRecord(req, "companies", company.id))
      .filter((company) =>
        getDataScope(req, "companies") === "own"
          ? company.createdById === req.session.user?.entraObjectId
          : true,
      );
    const filteredCompanies = visibleCompanies
      .filter((company) => matchesStatus(company.status, query.status))
      .filter((company) =>
        matchesSearch(
          [
            company.name,
    ...
    for (const contact of contacts) {
      if (!contact.companyId || !visibleCompanyIds.has(contact.companyId))
        continue;
    ...
    res.status(200).json({
      data: page.data.map((company) =>
        toCompanyDto(company, contactsByCompany.get(company.id) ?? [], req),
      ),
      meta: page.meta,
    });
  } catch (error) {
  ...
  try {
    ...
    if (hasResourceRestriction(req, "companies")) {
      res.status(403).json({
        success: false,
        message:
          "You cannot create companies while company access is restricted to assigned records.",
      });
      return;
    }
    const forbiddenField = firstHiddenInput(req, req.body, {
      ...
      customerSince: "companies.customerSince",
      tags: "companies.tags",
...
export async function updateCompany(
  ...
  try {
    ...
    await publishCrmEvent({
      tenantId: savedCompany.tenantId,
      resource: "companies",
      action: "updated",
      entityId: savedCompany.id,
      actorObjectId: req.session.user?.entraObjectId,
    ...
    res.status(200).json({
      data: toCompanyDto(
        savedCompany,
        contacts.map(toCompanyContactSummary),
        req,
      ),
    });
  } catch (error) {
  ...
  try {
    ...
    if (company.logoUrl) {
      await removeObject(company.logoUrl).catch((error) => {
        console.error(
          "Failed to remove company logo from object storage",
          error,
        );
      });
    }
...
export async function uploadCompanyLogo(
  ...
  try {
    if (!canAccessRecord(req, "companies", String(req.params.id))) {
      res
        .status(404)
        .json({ success: false, message: "Company logo not found." });
      return;
    }
    const company = await companyRepository().findOneBy({
    ...
    if (previousObjectKey) {
      await removeObject(previousObjectKey).catch((error) => {
        console.error(
          "Failed to remove previous company logo from object storage",
          error,
        );
      });
    }
