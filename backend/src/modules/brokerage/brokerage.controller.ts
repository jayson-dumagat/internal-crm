...
function hasRestrictedWrite(req: Request, res: Response, fields: Record<string, string>): boolean {
  const field = firstHiddenInput(req, req.body as Record<string, unknown>, fields);
  if (!field) return false;
  res.status(403).json({ success: false, message: `You cannot write the restricted field ${field}.` });
  return true;
}
...
function complianceDto(item: ComplianceCase, req?: Request) {
  const canSeeDetails = !req || canViewField(req, "compliance.details");
  return { ...item, description: canSeeDetails ? item.description : null, resolution: canSeeDetails ? item.resolution : null, assignedTo: userDto(item.assignedUser), createdAt: item.createdAt.toISOString(), updatedAt: item.updatedAt.toISOString(), resolvedAt: item.resolvedAt?.toISOString() ?? null };
}
...
export async function createBrokerageAccount(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    if (hasRestrictedWrite(req, res, { accountNumber: "brokerageAccounts.accountNumber", externalAccountId: "brokerageAccounts.externalId" })) return;
...
export async function listKycCases(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    res.status(200).json({ data: page.data.map((row) => kycDto(row, reviews.filter((review) => review.caseId === row.id), req)), meta: page.meta });
  } catch (error) { next(error); }
}
...
export async function createKycCase(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    if (hasRestrictedWrite(req, res, { identityVerification: "kyc.details", beneficialOwners: "kyc.details", authorizedRepresentatives: "kyc.details", sourceOfFunds: "kyc.details", purposeOfAccount: "kyc.details", pepStatus: "kyc.details", sanctionsStatus: "kyc.details", missingDocuments: "kyc.details", expiredDocuments: "kyc.details", enhancedDueDiligence: "kyc.details" })) return;
    ...
    await audit(req, "created KYC case", saved.id, "kyc", saved.id); res.status(201).json({ data: kycDto(saved, [], req) });
  } catch (error) { next(error); }
}
...
export async function updateKycCase(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    Object.assign(row, parsed.data, parsed.data.assignedReviewerId !== undefined ? { assignedReviewerId: reviewer?.id ?? null, assignedReviewer: reviewer } : {}); const saved = await repo(KycCase).save(row); await audit(req, "updated KYC case", saved.id, "kyc", saved.id); res.status(200).json({ data: kycDto(saved, [], req) });
  } catch (error) { next(error); }
}
